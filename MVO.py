...
class MeanVarOpt(bt.Strategy):
    ...
    params = (
        ...
        ('risk_tolerance', 1)  # Risk tolerance parameter for mean-variance optimization
    )
    ...
    def next(self):
        ...
        for i, d in enumerate(self.datas):
            if d._name in self.selected_stocks and start_dates[d._name] < curr_date - datetime.timedelta(days=self.params.lookback):
                ...
                no_of_stocks=portfolio_value*self.weights[i]/d.close[0]
                # if the number of stocks is NaN, buy 0 stocks
                try:
            ...
            # else:
            #     self.close(data=d)
            #     self.log(f"Closing {d._name} position")
